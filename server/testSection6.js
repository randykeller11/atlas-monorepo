...
async function testSection6Implementation() {
  console.log('=== Testing Section 6: Formalized State Machine for Assessment Flow ===\n');
  ...
  await runTest('State Machine Initialization', async () => {
    ...
    if (service.state.value !== 'introduction') {
      throw new Error(`Expected initial state 'introduction', got '${service.state.value}'`);
    }
    ...
    console.log(`     ✓ Initial context: ${JSON.stringify(service.state.context.sections)}`);
  });
  ...
  await runTest('State Transitions', async () => {
    ...
    let result = await recordResponse(testSessionId, { type: 'text', content: 'Test response' });
    
    if (result.currentState !== 'interestExploration') {
      throw new Error(`Expected transition to 'interestExploration', got '${result.currentState}'`);
    }
    
    // Test interestExploration -> workStyle (after 2 questions)
    await recordResponse(testSessionId, { type: 'multiple_choice', content: 'Test MC 1' });
    result = await recordResponse(testSessionId, { type: 'multiple_choice', content: 'Test MC 2' });
    
    if (result.currentState !== 'workStyle') {
      throw new Error(`Expected transition to 'workStyle', got '${result.currentState}'`);
    }
  ...
  await runTest('Question Type Enforcement', async () => {
    ...
    if (nextQuestion.type !== 'text') {
      throw new Error(`Introduction should require 'text', got '${nextQuestion.type}'`);
    }
    ...
    if (nextQuestion.type !== 'multiple_choice') {
      throw new Error(`Interest exploration should require 'multiple_choice', got '${nextQuestion.type}'`);
    }
    
    // Move to work style
    await recordResponse(testSessionId, { type: 'multiple_choice', content: 'Test MC 1' });
    await recordResponse(testSessionId, { type: 'multiple_choice', content: 'Test MC 2' });
  ...
  await runTest('State Persistence', async () => {
    ...
    // Get current state
    const service1 = await assessmentStateMachineService.getMachine(testSessionId);
    ...
    const service2 = await assessmentStateMachineService.getMachine(testSessionId);
    ...
    console.log(`     ✓ Restored state: ${state2}, questions: ${context2.totalQuestions}`);
  });
  ...
  await runTest('Complete Assessment Flow', async () => {
    ...
    for (let i = 0; i < responses.length; i++) {
      ...
      if (nextQuestion.type !== response.type) {
        throw new Error(`Question ${i + 1}: Expected ${response.type}, got ${nextQuestion.type}`);
      }
      
      const result = await recordResponse(testSessionId, response);
      currentState = result.currentState;
      
      console.log(`     Question ${i + 1}: ${response.type} -> ${currentState}`);
    }
  ...
  await runTest('State Validation', async () => {
    const validation = await assessmentStateMachineService.validateState(testSessionId);
    ...
    if (validation.context.totalQuestions !== 10) {
      throw new Error(`Expected 10 total questions, got ${validation.context.totalQuestions}`);
    }
    ...
    Object.entries(expectedTypes).forEach(([type, expected]) => {
      if (validation.context.questionTypes[type] !== expected) {
        throw new Error(`Expected ${expected} ${type} questions, got ${validation.context.questionTypes[type]}`);
      }
    });
    
    console.log(`     ✓ State validation passed`);
    console.log(`     ✓ Question distribution: ${JSON.stringify(validation.context.questionTypes)}`);
  });
  ...
  await runTest('Error Handling', async () => {
    ...
    try {
      await recordResponse(testSessionId, { type: 'multiple_choice', content: 'Wrong type' });
    ...
    // Submit correct type
    await recordResponse(testSessionId, { type: 'text', content: 'Correct type' });
  ...
  if (testsPassed === testsTotal) {
    console.log('🎉 Section 6 (Formalized State Machine for Assessment Flow) is fully implemented!');
    console.log('\n✅ Verified Features:');
    ...
    console.log('⚠ Some Section 6 features need attention. Check the errors above.');
